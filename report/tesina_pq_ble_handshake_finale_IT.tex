...
\newcommand{\ProjectDate}{21 luglio 2026}
...
% ../report/images/. Se il file manca, mostra un segnaposto compilabile.
\newcommand{\safeincludegraphics}[2][width=\linewidth]{%
  \IfFileExists{images/#2}{%
    \includegraphics[#1]{images/#2}%
  }{%
    \IfFileExists{../report/images/#2}{%
      \includegraphics[#1]{../report/images/#2}%
    }{%
      \fbox{\parbox[c][5.2cm][c]{0.92\linewidth}{%
        \centering\small\textcolor{black!60}{Immagine non trovata.\\[0.4em]
        File atteso: \texttt{\detokenize{#2}} in \texttt{images/} oppure \texttt{../report/images/}.}%
      }}%
    }%
  }%
}
...
\IfFileExists{benchmark_values.tex}{\input{benchmark_values.tex}}{}
...
\noindent\textbf{Nota storica.} Questa relazione descrive il primo prototipo
Python e la validazione del trasporto BLE/GATT. Risultati, limiti e sviluppi
futuri riportati nel testo si riferiscono a quella fase. Le baseline v0.7 e
v1.0 sono state successivamente completate; v1.0 termina a CP5. Anche la
\emph{Post-v1.0 Comparative Evaluation} è completa da EVAL-A a EVAL-E.
Per lo stato corrente e i risultati finali si vedano il README del repository
e le milestone EVAL-D ed EVAL-E in \texttt{docs/research/milestones/}.
...
protocollo è coperta da una suite attiva di \textbf{109 test automatizzati}, comprendente anche verifiche
riproducibili sulle dimensioni del materiale ML-KEM, sull'overhead
fisso del canale AES-256-GCM e sulla frammentazione applicativa
del ciphertext.
...
Nella configurazione validata, la funzione di frammentazione utilizza
una dimensione logica massima di 247 byte per frammento. Poiché 4 byte
sono riservati all'header applicativo, il payload utile per il
ciphertext è:
...
La suite attiva contiene \textbf{109 test passanti}.
...
\bottomrule\noalign{}
\endlastfoot
\passthrough{\lstinline!test\_ml\_kem.py!} & 6 & Dimensioni FIPS 203 di public key,
secret key, ciphertext e shared secret;
...
\passthrough{\lstinline!test\_protocol\_overhead.py!} & 8 &
Overhead fisso AES-GCM, dimensione del full handshake e messaggi di session resumption \\
\textbf{Totale} & \textbf{109} & Validazione proof-of-concept \\
\end{longtable}
...
SAS derivation & \BenchSasMean & \BenchSasMedian & \BenchSasPnn \\
HKDF-SHA256 & \BenchHkdfMean & \BenchHkdfMedian & \BenchHkdfPnn \\
\midrule
...
\begin{itemize}
    \item correttezza della logica ML-KEM/SAS/HKDF/AES-GCM in Python;
    \item verifica automatica delle dimensioni ML-KEM-768 definite da FIPS 203;
    \item replay protection e AAD binding;
    \item overhead fisso di 37 byte del SecureChannel;
    \item frammentazione del ciphertext ML-KEM-768 in cinque frammenti logici nella configurazione da 247 byte;
    \item materiale applicativo del full handshake pari a 2292 byte;
    \item session resumption lato Python con limiti di 24 ore e 100 resume riusciti;
    \item public key ML-KEM-768 valida nel firmware e verifica del fingerprint via BLE;
    \item benchmark riproducibili con output TXT e JSON;
    \item trasferimento reale via BLE/GATT tra PC e nRF54L15 DK;
    \item catturabilità e osservabilità del traffico con nRF52840 e Wireshark.
\end{itemize}
...
\begin{itemize}
\tightlist
\item implementazione Python modulare del protocollo;
\item ML-KEM-768 integrato tramite liboqs;
\item SAS Numeric Comparison;
\item session key derivation tramite HKDF-SHA256;
\item canale AES-256-GCM con AAD e replay protection;
\item session resumption lato Python;
\item 109 test automatizzati passanti, comprendenti verifiche delle dimensioni ML-KEM, dell'overhead AES-GCM e del materiale applicativo del full handshake;
\item firmware Zephyr per nRF54L15 DK compilato e flashato;
\item demo reale PC central $\leftrightarrow$ nRF54L15 DK completata;
\item notification raw da 57 byte ricevuta;
\item cattura Wireshark con nRF52840 Dongle che mostra il trasporto ATT/GATT.
\end{itemize}

I risultati mostrano inoltre che il principale costo dimensionale
post-quantum è concentrato nel full handshake. Nella configurazione
validata, public key, ciphertext e header applicativi richiedono
2292 byte. Una volta derivata la session key, ML-KEM non viene più
utilizzato per i messaggi ordinari: il canale procede tramite
AES-256-GCM con un overhead fisso di 37 byte per messaggio.
