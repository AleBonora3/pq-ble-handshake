...
La validazione del progetto è divisa in tre livelli.

### 1. Cryptographic validation — Python

La logica crittografica è implementata e testata in Python:
...
La test suite attiva contiene attualmente:

```text
101 passed
```

Alcuni test legacy sul parsing automatico degli UUID firmware sono stati commentati perché legati a un formato precedente di dichiarazione degli UUID nel firmware. La coerenza UUID è stata invece verificata tramite ispezione GATT reale e test hardware PC central ↔ nRF54L15 DK.

### 2. BLE/GATT hardware validation — nRF54L15 DK

Il repository include un firmware Zephyr per **nRF54L15 DK** che espone il servizio GATT custom PQ-BLE.
...
- compilato con nRF Connect SDK 3.0.0;
- flashato correttamente sul nRF54L15 DK;
- rilevato come `PQ-BLE-Device`;
- connesso da nRF Connect Mobile;
- connesso dal central Python su Windows;
- validato per read public key, write ciphertext frammentato, control write e notification.

La modalità attuale è una **hardware transport demo**: il firmware valida il trasporto BLE/GATT reale, ma non esegue ancora ML-KEM/AES-GCM on-chip.

### 3. Observational validation — nRF52840 Dongle

Il dongle nRF52840 è previsto come sniffer passivo con Wireshark/nRF Sniffer per osservare il traffico PC ↔ DK.

Obiettivo della cattura:

- osservare advertising e connessione;
- osservare ATT/GATT read della public key;
- osservare ATT/GATT write dei frammenti di ciphertext;
- osservare write del comando `START`;
- osservare Handle Value Notification dal DK.

La cattura Wireshark completa è ancora da produrre e includere nella tesina.
...
| Sniffer | nRF52840 Dongle | Wireshark / nRF Sniffer | Planned passive capture |
...
- Wireshark capture with nRF52840 Dongle is still planned.
...
- optional nRF52840 Dongle for passive sniffing;
- optional Wireshark + nRF Sniffer for Bluetooth LE.
...
### Step 4 — Optional Wireshark capture
...
| Object | Raw size | Fragment payload | Fragments |
|---|---:|---:|---:|
| `pk_A` | 1184 B | ATT long read | multiple read blobs |
| `ct` | 1088 B | 243 B | 5 |
...
- Wireshark capture with the nRF52840 Dongle is still to be added as observational evidence.
...
- Wireshark capture with nRF52840 Dongle.
- Screenshots and packet analysis for the tesina.
